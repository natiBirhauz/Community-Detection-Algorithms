#include all the nodes for all the tests
...
def read_and_filter_graph(filename, folder_path='facebook100'):
...
def run_girvan_newman(G):
    comp = algorithms.girvan_newman(G)
    return comp
...
for file_name in os.listdir(data_folder):
    if file_name.endswith(".mat"):
        try:
            G, local_info = read_and_filter_graph(file_name, folder_path=data_folder)
        ...
        for k in range(2, 6):
            ...
            result_list.append({
                "file_name": file_name,
                "algorithm": "Clique Percolation",
                "clique_percolation_k": k,
